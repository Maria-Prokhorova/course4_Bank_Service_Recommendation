...
@Component
public class StaticRuleSimpleCreditImp implements StaticRule {

    private final StaticQueryExecutorImpl staticQueryExecutor;

    public StaticRuleSimpleCreditImp(StaticQueryExecutorImpl staticQueryExecutor) {
    ...
    /**
     * Внутренний метод, который проверяет, чтобы весь набор правил по продукт "Простой кредит" соблюдался.
     *
     * @param userId - ID клиента.
    ...
    private boolean isEligibleForSimpleCreditRule(UUID userId) {
        return !staticQueryExecutor.isUsingProduct(
                StaticRuleParameters.of(userId, TYPE_CREDIT))
                && staticQueryExecutor.isAmountDepositsMoreThanWithdrawals(
                StaticRuleParameters.of(userId, TYPE_DEBIT))
                && staticQueryExecutor.isAmountWithdrawMoreLimit(
                StaticRuleParameters.of(userId, TYPE_DEBIT, LIMIT_SIMPLE_CREDIT));
    }
